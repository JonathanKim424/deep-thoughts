...
db.once('open', async () => {
  ...
  for (let i = 0; i < 100; i += 1) {
    ...
    const randomUserIndex = Math.floor(Math.random() * Object.keys(createdUsers.insertedIds).length);
    const { username, _id:userId } = await User.findOne({ _id: createdUsers.insertedIds[`${randomUserIndex}`] })
  ...
  for (let i = 0; i < 100; i += 1) {
    ...
    const randomUserIndex = Math.floor(Math.random() * Object.keys(createdUsers.insertedIds).length);
    const { username } = await User.findOne({ _id: createdUsers.insertedIds[`${randomUserIndex}`] });
